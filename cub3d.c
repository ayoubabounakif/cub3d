...
/*   Updated: 2020/12/14 19:28:32 by aabounak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		cast_horz(float ray_angle)
{
	...
	g_ray.yintercept += g_ray.isray_down ? TILE_SIZE : 0;
	g_ray.xintercept = g_player.x + (g_ray.yintercept - g_player.y) / tan(ray_angle);
	...
	g_ray.next_htouch_x = g_ray.xintercept;
	g_ray.next_htouch_y = g_ray.yintercept;
...
void		cast_vert(float ray_angle)
{
	...
	g_ray.xintercept += g_ray.isray_right ? TILE_SIZE : 0;
	g_ray.yintercept = g_player.y + (g_ray.xintercept - g_player.x) * tan(ray_angle);
	...
	g_ray.next_vtouch_x = g_ray.xintercept;
	g_ray.next_vtouch_y = g_ray.yintercept;
...
void		store_data(int strip_id, float ray_angle)
{
	...
	g_ray.isray_right = ray_angle < 0.5 * M_PI || ray_angle > 1.5 * M_PI;
	...
	: (float)INT32_MAX;
	g_rays[strip_id].distance = (g_ray.horz_hit_dist < g_ray.vert_hit_dist) ? g_ray.horz_hit_dist : g_ray.vert_hit_dist;
	g_rays[strip_id].wall_hit_x = (g_ray.horz_hit_dist < g_ray.vert_hit_dist) ? g_ray.horz_hit_x : g_ray.vert_hit_x;
	g_rays[strip_id].wall_hit_y = (g_ray.horz_hit_dist < g_ray.vert_hit_dist) ? g_ray.horz_hit_y : g_ray.vert_hit_y;
	g_rays[strip_id].was_hit_vert = (g_ray.vert_hit_dist < g_ray.horz_hit_dist) ? TRUE : FALSE;
	...
	g_rays[strip_id].isray_right = ray_angle < 0.5 * M_PI || ray_angle > 1.5 * M_PI;
	g_rays[strip_id].isray_left = !(ray_angle < 0.5 * M_PI || ray_angle > 1.5 * M_PI);
}
